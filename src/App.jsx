  ...
  const fetchAllData = async (orgId) => {
    ...
    try {
      const [
        ...
        supabase.from('expense_entries').select('*').eq('organization_id', orgId).is('deleted_at', null).order('date', { ascending: false }),
        supabase.from('income_sources').select('*').eq('organization_id', orgId).is('deleted_at', null),
        supabase.from('fixed_expenses').select('*').eq('organization_id', orgId).is('deleted_at', null),
        supabase.from('fixed_expense_amount_history').select('*'),
        supabase.from('children').select('*').eq('organization_id', orgId).is('deleted_at', null),
        supabase.from('school_years').select('*').eq('organization_id', orgId).is('deleted_at', null),
        supabase.from('installments').select('*').eq('organization_id', orgId).is('deleted_at', null).order('month', { ascending: true }),
        supabase.from('goals').select('*').eq('organization_id', orgId).is('deleted_at', null),
