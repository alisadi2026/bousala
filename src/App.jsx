...
export default function App() {
  ...
  const [programForm, setProgramForm] = useState({ slug: '', name: '', description: '', icon: '📦', color: '#C9A24B', route: '', sort_order: 0 });
    ...
    if (prof) {
      ...
      if (prof.is_super_admin) {
        try {
          ...
          // Fetch available programs for this user
          try {
            ...
            console.log('Programs fetch failed, fallback to bousala only:', e);
  ...
  async function addGoal() {
    ...
    const { data, error } = await supabase.from('goals').insert({
      ...
      fixed_amount: parseFloat(goalForm.fixedAmount) || 0,
  ...
  async function updateSaved(id, value) {
    const savedVal = parseFloat(value) || 0;
    const { error } = await supabase.from('goals').update({ saved: savedVal }).eq('id', id);
    if (error) return showToast(error.message);
    setGoals((prev) => prev.map((g) => (g.id === id ? { ...g, saved: savedVal } : g)));
  }
  ...
  async function addFixed() {
    ...
    const { data, error } = await supabase.from('fixed_expenses').insert({
      ...
      end_date: fixedForm.endDate || null
    }).select().single();
    if (error) return showToast(error.message);
    // Insert initial history
    await supabase.from('fixed_expense_amount_history').insert({
      fixed_expense_id: data.id,
      amount: amount,
      effective_from: fixedForm.startDate
    });
    setFixedExpenses((prev) => [
      ...prev,
      {
        ...
        amount: Number(data.current_amount),
        startDate: data.start_date,
        endDate: data.end_date,
        amountHistory: [{ id: `${data.id}-a0`, amount: Number(data.current_amount), effectiveFrom: data.start_date }],
      },
    ]);
    setFixedForm({ name: "", category: DEFAULT_CATEGORIES[0]?.key || "أخرى", amount: "", startDate: todayISO(), endDate: "" });
  ...
  async function addChild() {
    ...
    if (error) return showToast(error.message);
    setChildren((prev) => [...prev, { id: data.id, name: data.name, years: [] }]);
    setChildForm({ name: "" });
  ...
  async function addChildYear() {
    ...
    if (!currentOrgId) return showToast("لا يوجد منظمة");
    if (!yearFormFor) return showToast("لا يوجد طفل محدد");
    
    const annualFee = parseFloat(yearForm.annualFee) || 0;
    const downPayment = parseFloat(yearForm.downPayment) || 0;
    const installmentsCount = parseInt(yearForm.installmentsCount) || 10;
    
    const { data: yearData, error: yearErr } = await supabase.from('school_years').insert({
      child_id: yearFormFor,
      organization_id: currentOrgId,
      label: yearForm.label.trim() || (yearForm.stage === "university" ? t("stageUniversity") : t("stageSchool")),
      stage: yearForm.stage,
      annual_fee: annualFee,
      down_payment: downPayment,
      down_payment_date: yearForm.downPaymentDate || null,
      installments_count: installmentsCount,
      start_date: yearForm.startDate
    }).select().single();
    
    if (yearErr) return showToast(yearErr.message);
    
    // Create installments
    const remaining = annualFee - downPayment;
    ...
    if (downPayment > 0) {
      installmentsToInsert.push({
        year_id: yearData.id,
        organization_id: currentOrgId,
        month: yearData.down_payment_date || yearData.start_date,
        amount: downPayment,
        paid: false,
        paid_amount: 0,
    ...
    for (let i = 0; i < installmentsCount; i++) {
      const instDate = addCalendarMonths(yearData.start_date, i);
      installmentsToInsert.push({
        year_id: yearData.id,
        organization_id: currentOrgId,
        month: instDate,
    ...
    const { data: instData, error: instErr } = await supabase.from('installments').insert(installmentsToInsert).select();
    if (instErr) return showToast(instErr.message);
    
    const newYear = {
      id: yearData.id,
      label: yearData.label,
      stage: yearData.stage,
      annualFee: Number(yearData.annual_fee),
      downPayment: Number(yearData.down_payment),
      downPaymentDate: yearData.down_payment_date,
      installmentsCount: yearData.installments_count,
      startDate: yearData.start_date,
      installments: (instData||[]).map(ins=>({
        id: ins.id,
        month: ins.month,
        amount: Number(ins.amount),
        paid: ins.paid,
    ...
    setChildren((prev) => prev.map((c) => (c.id === yearFormFor ? { ...c, years: [...c.years, newYear] } : c)));
    closeYearForm();
  ...
  return (
    <div dir={dir} style={{ background: INK, minHeight: "100vh", fontFamily: "'Tajawal','Segoe UI',Tahoma,sans-serif", color: PAPER, padding: "28px 18px 60px" }}>
          ...
          {availablePrograms.length>0 && (
            <div style={{display:"flex", alignItems:"center", gap:6, background:CARD_SOFT, borderRadius:9, padding:"6px 10px", border:`1px solid ${LINE}`}}>
              ...
              <select className="field" value={selectedProgram} onChange={e=>{
                ...
                localStorage.setItem('bousala_program', newProg);
      ...
      {/* Programs Management - Super Admin Only */}
        ...
        <Modal title="إدارة البرامج / الموديولز - Super Admin" onClose={()=>setShowProgramsManagement(false)} dir={dir}>
          <div style={{display:"flex", flexDirection:"column", gap:14}}>
            ...
            <div className="card" style={{padding:14, border:`1px solid ${GOLD}`}}>
              ...
              <button className="btn" onClick={async()=>{
                if (!programForm.slug || !programForm.name) return showToast('أدخل slug واسم البرنامج');
                try {
                  ...
                  setProgramForm({ slug: '', name: '', description: '', icon: '📦', color: '#C9A24B', route: '', sort_order: 0 });
                ...
                <div key={p.id} className="card" style={{padding:12, border:`1px solid ${p.is_active ? TEAL : LINE}`, background: p.is_active ? `${TEAL}0A` : CARD_SOFT}}>
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:8, flexWrap:"wrap"}}>
                    <div style={{display:"flex", alignItems:"center", gap:10}}>
                      ...
                      <div>
                        <div style={{fontWeight:800, fontSize:12, display:"flex", alignItems:"center", gap:6}}>{p.name} <span style={{fontSize:10, color:MUTED}}>({p.slug})</span> {p.is_active ? <span style={{background:`${TEAL}22`, color:TEAL, fontSize:9, padding:"2px 5px", borderRadius:999}}>✅ مفعل</span> : <span style={{background:`${RED}22`, color:RED, fontSize:9, padding:"2px 5px", borderRadius:999}}>❌ معطل</span>}</div>
                  ...
                  <div style={{marginTop:8, display:"flex", gap:6, flexWrap:"wrap"}}>
                    {allOrgs.slice(0,5).map(org=>(
                      <button key={org.id} className="btn" onClick={async()=>{
                        try {
                          const { error } = await supabase.rpc('admin_assign_program_to_org', { target_org_id: org.id, target_program_slug: p.slug, is_active: true });
                          if (error) throw error;
                          showToast(`تم إعطاء ${p.name} لـ ${org.name}`);
                        } catch (err) { showToast(err.message); }
                      }} style={{background:`${GOLD}15`, border:`1px solid ${GOLD}33`, color:GOLD, borderRadius:6, padding:"3px 6px", fontSize:9}}>{org.name.slice(0,10)} +{p.icon}</button>
                    ))}
                  </div>
                </div>
              ))}
      ...
      {/* Programs Launcher - Shows when user has multiple programs */}
      ...
      {selectedProgram !== 'bousala' && (
        <div className="card" style={{ maxWidth: 980, margin: "0 auto 18px", padding: 30, textAlign:"center", borderColor: GOLD }}>
          ...
          }} style={{marginTop:16, background:GOLD, color:INK, borderRadius:8, padding:"8px 16px", fontWeight:700}}>العودة لبوصلة 🧭</button>
        </div>
      )}
