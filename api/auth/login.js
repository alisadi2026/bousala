...
function getClient() {
  if (!supabaseUrl || !serviceKey) throw new Error('Missing SUPABASE env - set SUPABASE_SERVICE_ROLE_KEY in Vercel');
...
export default async function handler(req, res) {
  ...
  return res.status(200).json({ 
    token, 
    user: { id: profile.id, email: profile.email, username: profile.username, full_name: profile.full_name, is_super_admin: profile.is_super_admin }
  });
}
