...
export default async function handler(req, res) {
  ...
  if (!email ||!password) return res.status(400).json({ error: 'Email required' });
  ...
  const { error } = await supabase.from('profiles').insert({ id: userId, email: lowerEmail, username: lowerEmail.split('@')[0], full_name: lowerEmail.split('@')[0], password_hash: hash, is_super_admin: false });
  ...
  return res.status(200).json({ token, user: { id: userId, email: lowerEmail, username: lowerEmail.split('@')[0], is_super_admin: false } });
}
